...
#include <sstream>
#include "crypto-browserify.bytecode.h"
#include "js.h"

namespace cryptofuzz {
namespace module {

crypto_browserify::crypto_browserify(void) :
    Module("crypto-browserify"),
    js(new JS()) {

    const std::vector<uint8_t> bc(crypto_browserify_bytecode, crypto_browserify_bytecode + crypto_browserify_bytecode_len);
...
std::optional<component::Digest> crypto_browserify::OpDigest(operation::Digest& op) {
    return Run<component::Digest, operation::Digest, CF_OPERATION("Digest")>((JS*)js, op);
}

std::optional<component::MAC> crypto_browserify::OpHMAC(operation::HMAC& op) {
    return Run<component::MAC, operation::HMAC, CF_OPERATION("HMAC")>((JS*)js, op);
}

std::optional<component::Ciphertext> crypto_browserify::OpSymmetricEncrypt(operation::SymmetricEncrypt& op) {
    return Run<component::Ciphertext, operation::SymmetricEncrypt, CF_OPERATION("SymmetricEncrypt")>((JS*)js, op);
}

std::optional<component::Cleartext> crypto_browserify::OpSymmetricDecrypt(operation::SymmetricDecrypt& op) {
    return Run<component::Cleartext, operation::SymmetricDecrypt, CF_OPERATION("SymmetricDecrypt")>((JS*)js, op);
}

std::optional<component::Key> crypto_browserify::OpKDF_HKDF(operation::KDF_HKDF& op) {
    return Run<component::Key, operation::KDF_HKDF, CF_OPERATION("KDF_HKDF")>((JS*)js, op);
}

std::optional<component::Key> crypto_browserify::OpKDF_PBKDF2(operation::KDF_PBKDF2& op) {
    return Run<component::Key, operation::KDF_PBKDF2, CF_OPERATION("KDF_PBKDF2")>((JS*)js, op);
}

std::optional<component::Key> crypto_browserify::OpKDF_SCRYPT(operation::KDF_SCRYPT& op) {
...
std::optional<component::Bignum> crypto_browserify::OpBignumCalc(operation::BignumCalc& op) {
...
std::optional<component::ECC_PublicKey> crypto_browserify::OpECC_PrivateToPublic(operation::ECC_PrivateToPublic& op) {
