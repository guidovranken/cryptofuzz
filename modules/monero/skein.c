...
static HashReturnSkein Init  (hashState *state, int hashbitlen);
static HashReturnSkein Update(hashState *state, const BitSequence *data, DataLengthSkein databitlen);
static HashReturnSkein Final (hashState *state,       BitSequence *hashval);
...
HashReturnSkein skein_hash(int hashbitlen, const BitSequence *data, /* all-in-one call */
                DataLengthSkein databitlen,BitSequence *hashval)
{
  hashState  state;
  HashReturnSkein r = Init(&state,hashbitlen);
  if (r == SKEIN_SUCCESS)
